...
export function CapacityModelConsole(){
  ...
  async function load(){
    ...
    try{
      ...
      setSettings(sr); setResources(rr); setPaintRules(pr);
      ...
      setRuleConditionEdit(Object.fromEntries(pr.map(x=>[x.id,JSON.stringify(x.condition_json,null,2)])));
      setRuleActionEdit(Object.fromEntries(pr.map(x=>[x.id,JSON.stringify(x.action_json,null,2)])));
  ...
  return <div className="stack">
    ...
    <section className="panel">
      ...
      <div className="output-config-note"><strong>Configuration-first scheduling · v019 Chemical + Painting detailed models active</strong><span>Chemical Line uses Loading → Process → NDT → Unloading on 6 Flybars. Wet painting uses Setup → Application → Flash/Wait → Cure → Release, selects eligible CAB1–CAB4 by operation/recipe rule, and still preserves the configured total process time. All assumptions remain editable here.</span></div>
