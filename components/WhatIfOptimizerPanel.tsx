...
export function WhatIfOptimizerPanel({targetDate,cutoffTime,targetValue}:{targetDate:string;cutoffTime:string;targetValue:number}){
  ...
  return <section className="panel whatif-panel">
    ...
    {data?<>
      ...
      <div className="output-config-note"><strong>Optimizer result is advisory</strong><span>Best scenario is selected by smallest target gap, then highest confirmed forecast, then fewer unscheduled/late batches. No scenario changes Resource Master, Batch Plan or Scheduling data.</span></div>
