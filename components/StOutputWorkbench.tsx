...
type Step = {
  routePosition:number; routeOccurrenceKey:string; stepType:"PLANNING"|"INTERMEDIATE_INSPECTION"|"PHYSICAL_SUPPORT"; operationCode:string; operationSequence:number|null;
  ...
  durationMinutes:number|null; durationBasis:string; state:string; batchNo:string|null; batchStatus:string|null;
  predictiveP50Minutes:number|null; predictiveP80Minutes:number|null; predictiveSampleCount:number; etaConfidence:"HIGH"|"MEDIUM"|"LOW"|"NONE"; etaBasis:string;
...
export function StOutputWorkbench(){
  ...
  return <div className="stack st-output-workbench">
    ...
    <section className="panel output-command-panel">
      ...
      <div className="output-policy-line"><span>FINAL GATES</span><strong>{data?.finalGateCodes?.join(" / ")||data?.endpointOperation||"FINSST / CFINM-VN"}</strong><span>CUTOFF</span><strong>{cutoffTime}</strong><span>ROUTE SNAPSHOT</span><strong>{fmtTime(data?.routeSnapshotAt)}</strong><span>LOGIC</span><strong>Actual → Reservation → P50/P80 History → Final ETA → Unique Job dm²</strong></div>
    </section>
