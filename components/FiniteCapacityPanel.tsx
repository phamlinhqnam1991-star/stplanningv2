...
type Segment={kind:string;label:string;startAt:string;endAt:string;durationMinutes:number};
type Batch = {
...
export function FiniteCapacityPanel({targetDate,cutoffTime,targetValue}:{targetDate:string;cutoffTime:string;targetValue:number}){
  ...
  return <section className="panel finite-capacity-panel">
    <div className="panel-head"><div><span className="eyebrow">FINITE CAPACITY / OUT-41</span><h2>Proposed Batch → Resource Timeline → Target Feasibility</h2><p className="subcell">Simulates current schedule occupancy, existing unscheduled batches and new proposed batches before confirming the ST Output target. Chemical Line uses segmented Loading → Process → NDT → Unloading logic on 6 Flybars with process concurrency and NDT-start spacing.</p></div><button className="button primary" disabled={loading} onClick={()=>void run()}>{loading?"Simulating…":"Run Finite Capacity"}</button></div>
